...
setwd("~./MeioticRecombSexualDimorphism/")

#load(file = "~./MeioticRecombSexualDimorphism/data/MLH1_data_setup_10.11.20.RData")
load(file = "~./MeioticRecombSexualDimorphism/data/File_S7.RData")

#load functions
#source("~./MLH1repo/src/CommonFunc_MLH1repo.R")
source("~./MeioticRecombSexualDimorphism/data/File_S10.R" )
```

<!-- Data Set Up Notes -->

Analysis sections are organized by meiotic traits. Running this file will write out .xlsx files for the results of each of the models. These results are found it Supplemental_Table1.xslx


Sections in the lower part are organized into 2 main parts: 
Q1- analyses focused on the sex differences in meiotic traits. 
Q2- analyses focused on differences male meiotic traits from the high and low Mus musculus groups.
...
DMC1.DF$total.count<- as.character(DMC1.DF$total.count)
DMC1.DF$total.count <- as.numeric(DMC1.DF$total.count)


#make DMC1 table
DMC1.mean.obs.L <- ddply(.data=DMC1.DF[DMC1.DF$stage == "Late",],
                 .(category),
                 summarize, 
...
DMC1.mean.obs.Z <- ddply(.data=DMC1.DF[DMC1.DF$stage == "Early",],
                 .(category),
                 summarize, 
                 ncells = length(fileName),
                 mean.count = mean( total.count)
...
```{r Q1_SC_Long.M1, echo=FALSE, eval=TRUE}

Long.biv_mouse_table$subsp <- factor(Long.biv_mouse_table$subsp, ordered = FALSE)
Long.biv_mouse_table$sex <- factor(Long.biv_mouse_table$sex, ordered = FALSE)
Long.biv_mouse_table$strain <- factor(Long.biv_mouse_table$strain, ordered = FALSE)
#Long.biv_mouse_table$species <- factor(Long.biv_mouse_table$species, ordered = FALSE)

Q1.lmer_long_M1 <- lmer(mean_SC ~ subsp * sex + (1|strain), data= ( Short.biv_mouse_table %>% filter(species == "M.musculus") %>% filter(strain !=  "CAST" ) %>% filter(strain !=  "MOLF" ) %>% filter(strain !=  "HMI" ) %>% filter(strain !=  "AST" ) %>% filter(strain !=  "PERC" ) %>% filter(strain !=  "CZECH" ) %>% filter(strain !=  "TOM" ) ) )

Q1.lmer_long_M1_R.sex <- lmer(mean_SC ~ subsp + (1|strain), data= ( Short.biv_mouse_table %>%  filter(species == "M.musculus") %>% filter(strain !=  "CAST" ) %>% filter(strain !=  "MOLF" ) %>% filter(strain !=  "HMI" ) %>% filter(strain !=  "AST" ) %>% filter(strain !=  "PERC" ) %>% filter(strain !=  "CZECH" ) %>% filter(strain !=  "TOM" ) ) )

Q1.lmer_long_M1_R.subsp <- lmer(mean_SC ~ sex + (1|strain), data= ( Short.biv_mouse_table %>% filter(species == "M.musculus") %>% filter(strain !=  "CAST" ) %>% filter(strain !=  "MOLF" ) %>% filter(strain !=  "HMI" ) %>% filter(strain !=  "AST" ) %>% filter(strain !=  "PERC" ) %>% filter(strain !=  "CZECH" )  %>% filter(strain !=  "TOM" ) )  ) 


Q1.lmer_long_M1_sex <- anova(Q1.lmer_long_M1, Q1.lmer_long_M1_R.sex)$`Pr(>Chisq)`[2]

Q1.lmer_long_M1_subsp <- anova(Q1.lmer_long_M1, Q1.lmer_long_M1_R.subsp)$`Pr(>Chisq)`[2]

Q1.lmer_long_M1_int <- drop1(Q1.lmer_long_M1, test="Chisq")$`Pr(Chi)`[2]

Q1.lmer_long_M1_strain <- exactRLRT(Q1.lmer_long_M1)$p.value


Q1.lmer_long_M1_results <- data.frame(sub.sp = Q1.lmer_long_M1_subsp, 
sex.results =  Q1.lmer_long_M1_sex, 
               interaction.pval = Q1.lmer_long_M1_int,
rand.pval = Q1.lmer_long_M1_strain)


write.table(Q1.lmer_long_M1_results, "~./MLH1repo/LongSC_Q1_M1.csv", sep=",",row.names = TRUE)

#write_xlsx(Q1.lmer_short_M1_results, "~./MLH1repo/ShortSC_Q1_M1.xlsx", col_names = TRUE)
#write.table(ll$coefficients, "~./MLH1repo/LongSC_Q1_M1_coFIXED.csv", col.names = TRUE)

```


```{r Q1.SC_Long_M2, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}

Long.biv_mouse_table$subsp <- factor(Long.biv_mouse_table$subsp, ordered = FALSE)
Long.biv_mouse_table$sex <- factor(Long.biv_mouse_table$sex, ordered = FALSE)
Long.biv_mouse_table$strain <- factor(Long.biv_mouse_table$strain, ordered = FALSE)

Q1.glm_Long.SC_M2 <- glm(mean_SC ~ subsp * strain * sex, data= ( Long.biv_mouse_table %>% filter(strain !=  "CAST" ) %>% filter(strain !=  "MOLF" ) %>% filter(strain !=  "HMI" ) %>% filter(strain !=  "AST" ) %>% filter(strain !=  "PERC" ) %>% filter(strain !=  "CZECH" ) %>% filter(strain !=  "TOM" ) ) )

summ.Q1.glm_Long.SC_M2 <- summary(Q1.glm_Long.SC_M2)
#summ.Q1.glm_Long.SC_M2$coefficients[,c(1,4)][8,]

#sex most sig
#skive, slightly sig

write.table(summ.Q1.glm_Long.SC_M2$coefficients, "~./MLH1repo/LongSC_Q1_M2.csv", sep=",",row.names = TRUE)


write.xlsx( as.data.frame( summ.Q1.glm_Long.SC_M2$coefficients) , "ShortSC_Q1_M2.xlsx", col.names = TRUE, row.names = TRUE)
```


```{r Q1.glm_Long.SC_M3, echo=FALSE, eval=TRUE, message=FALSE}

Long.biv_mouse_table$subsp <- factor(Long.biv_mouse_table$subsp, ordered = FALSE)
Long.biv_mouse_table$sex <- factor(Long.biv_mouse_table$sex, ordered = FALSE)
Long.biv_mouse_table$strain <- factor(Long.biv_mouse_table$strain, ordered = FALSE)

Q1.glm_Long.SC_M3 <- glm(mean_SC ~ strain * sex, data= ( Long.biv_mouse_table %>% filter(strain !=  "CAST" ) %>% filter(strain !=  "MOLF" ) %>% filter(strain !=  "HMI" ) %>% filter(strain !=  "AST" ) %>% filter(strain !=  "PERC" ) %>% filter(strain !=  "CZECH" ) %>% filter(strain !=  "TOM" ) )  )

#summary(Q1.glm_Long.SC_M3)
summ.Q1.glm_Long.SC_M3 <- summary(Q1.glm_Long.SC_M3)

#round(summ.Q1.glm_Long.SC_M3$coefficients[,c(1,4)],5)
#sex male very sig -- only sig
#sum.Q1.glm_totSC_M2$coefficients[,c(1,4)][8,]
#SKIVE * male

write.table(summ.Q1.glm_Long.SC_M3$coefficients, "~./MLH1repo/LongSC_Q1_M3.csv", sep=",",row.names = TRUE)

write.xlsx(as.data.frame( summ.Q1.glm_Long.SC_M3$coefficients), "ShortSC_Q1_M3.xlsx", col.names = TRUE, row.names = TRUE)

```


```{r Female.Long.Bivalent.M4.M5, eval=FALSE, echo=FALSE}
#these M4 and M5 could be run for short and Long
Short.biv_mouse_table$subsp <- factor(Short.biv_mouse_table$subsp, ordered = FALSE)
Short.biv_mouse_table$sex <- factor(Short.biv_mouse_table$sex, ordered = FALSE)
Short.biv_mouse_table$strain <- factor(Short.biv_mouse_table$strain, ordered = FALSE)
...
```{r male.Long.Biv.M4.M5, echo=FALSE, eval=FALSE}
#M4
ShortBiv.male.glm_M4 <- glm(mean_SC ~ subsp * strain, data= (Short.biv_mouse_table %>% filter(subsp != "Cast") %>% filter(species == "M.musculus") %>% filter(sex == "male") ) )
sum.ShortBiv.male.glm_M4 <- summary(ShortBiv.male.glm_M4)
write.xlsx(as.data.frame( sum.ShortBiv.male.glm_M4$coefficients), "ShortBiv.male.glm_M4.xlsx", col.names = TRUE, row.names = TRUE)
...
#There is no difference in average position of MLH1 foci on bivalents with a single focus between high-recombination and low-recombination strains (t-test; p = 0.68). These results suggest that variation among strains in genome-wide recombination rate is connected to changes in crossover interference
...
#High-recombination strains have greater inter-focal distances than low-recombination strains (t-test; IFDnorm: p = #3.26^{-4} x10-4 ; IFDraw: p = 2.06^{-4} x10-4). 
...
#Percentages of IFDnorm values that fall below 0.3 range from 8.2% (domesticusG) to 16% (musculusKAZ) in low-recombination strains, whereas high-recombination strains show percentages below 4% (0%, 1.3%, and 3.3% for musculusSKIVE, molossinusMSM, and musculusPWD, respectively).
...
#t-tests of the difference of close vs long IFD rates
#High-recombination strains have greater inter-focal distances than low-recombination strains (t-test; IFDnorm: p = #3.26^{-4} x10-4 ; IFDraw: p = 2.06^{-4} x10-4). 
...
#save Rdata
#save.image("~./MeioticRecombSexualDimorphism/data/Results_setup_10.11.20.RData")

save.image("~./MeioticRecombSexualDimorphism/data/updated_S9.RData")
