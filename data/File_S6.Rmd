...
opts_knit$set(root.dir = "~./MeioticRecombSexualDimorphism/")
setwd("~./MeioticRecombSexualDimorphism/")

meta.data = read.csv( "~./MeioticRecombSexualDimorphism/data/clean.Meta.Data.txt", sep = "\t", header=TRUE)

#original file name
#final file name

#MLH1_data = read.xlsx2("~./MeioticRecombSexualDimorphism/data/MLH1.AnonData_formated.xlsx", header=TRUE, 1)
MLH1_data = read.xlsx2("~./MeioticRecombSexualDimorphism/data/File_S1.xlsx", header=TRUE, 1)

#DMC1.DF <- read.xlsx2("~./MeioticRecombSexualDimorphism/data/DMC1_data.xlsx", header = TRUE, 1)
DMC1.DF <- read.xlsx2("~./MeioticRecombSexualDimorphism/data/File_S3.xlsx", header = TRUE, 1)

#Total.SC_DF = read.xlsx2("~./MeioticRecombSexualDimorphism/data/Total.SC_DF_5.31.20.xlsx", header=TRUE, 1)
Total.SC_DF = read.xlsx2("~./MeioticRecombSexualDimorphism/data/File_S4.xlsx", header=TRUE, 1)

#Curated_BivData <-  read.xlsx2("~./MeioticRecombSexualDimorphism/data/Curated_BivData_5.30.20.xlsx", sep=" ", header=TRUE, 1)
Curated_BivData <-  read.xlsx2("~./MeioticRecombSexualDimorphism/data/File_S5.xlsx", sep=" ", header=TRUE, 1)


#source("~./MeioticRecombSexualDimorphism/data/CommonFunc_MLH1repo.R" )
...
colors_of_strains <- c('WSB'= "#56B4E9",'G' ='cadetblue','LEW'= 'lightblue','PERC'= 'blue',
                       ...
                       'MSM'= 'deeppink', 'MOLF'= 'hotpink',
                       
                    'CAST'='green','HMI'= 'forestgreen',
                    
                    'SPRET'='gold','SPI'= 'goldenrod1',
                    'CAROLI'='gold3',
...
#juvi mice ~ 2 weeks old, used for DMc1 only
juvi_mice.list <- c(
...
`r length(juvi_mice.list)` juvinile mice were dissected, mostly only for DMC1 counts. Few had good MLH1 cells so they were removed from the main MLH1_DF. 
...
```{r HQ.mouse.table, echo=FALSE, warning=TRUE}
...
```{r merge.meta_MLH1, echo=FALSE, warning=FALSE}
...
```{r Q12_mouse.table, echo=FALSE, warning=FALSE}
...
```{r DMC1.setup, echo=FALSE, warning=FALSE}
#load DMC1 csv
...
#RUN the factor -> chr -> num for cleanCurated_BivData 
#these introduce NAs
cleanCurated_BivData$hand.foci.count <- as.character(cleanCurated_BivData$hand.foci.count)
cleanCurated_BivData$chromosomeLength<- as.character(cleanCurated_BivData$chromosomeLength)
cleanCurated_BivData$boxNumber<- as.character(cleanCurated_BivData$boxNumber)
#cleanCurated_BivData$boxs.IDd<- as.character(cleanCurated_BivData$boxs.IDd)

cleanCurated_BivData$hand.foci.count <- as.numeric(cleanCurated_BivData$hand.foci.count)
cleanCurated_BivData$chromosomeLength<- as.numeric(cleanCurated_BivData$chromosomeLength)
cleanCurated_BivData$boxNumber<- as.numeric(cleanCurated_BivData$boxNumber)
#cleanCurated_BivData$boxs.IDd<- as.numeric(cleanCurated_BivData$boxs.IDd)
...
cell.level.BivData_table <- ddply(.data=cleanCurated_BivData, 
                       .(fileName),
                       summarize, 
                       boxs.IDd = max(boxNumber),
                       nboxes_passed = length(boxNumber),
...
pass.rate.hist <- ggplot(cell.level.BivData_table, aes(x = nboxes_passed) )+geom_histogram(binwidth = 1, alpha=.6) + facet_wrap(~category)+ggtitle("number of objects passed per cell")
pass.rate.hist
...
I want to make a dataset of 'longest' and 'shortest' bins of bivalents (1st and 4th quartille) from cells with at least 16 segmented bivalents from a single cell. 
...
The above chunk makes the MELT table (*MELT_Curated_BivData*), which can be used for Foci position comparison.
...
#Saves to File_S7.RData, uncomment to override
save.image("~./MeioticRecombSexualDimorphism/data/updateFile_S7.RData")
